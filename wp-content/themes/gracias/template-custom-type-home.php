<?php
/*  Template Name: Custom Type Home

	Custom template that combines the content of a Page with a
	multi-column, archive-style display of the posts from a custom
	post type.
...
<?php get_header(); ?>
	...
	<div id="container">
		...
		<!-- Display posts per the assigned custom type -->
				...
				<div class="entries">
					<?php while( $boxes->have_posts() ) : $boxes->the_post(); ?>
						<?php
							...
							if ($post_type == 'post')  get_template_part('content', get_post_format());
							else get_template_part('content', $post_type);
						?>
					<?php endwhile; ?>
				</div><!-- .entries -->
