<?php
/*  Template Name: Landing Page (Colored Shadows)

	Custom "landing page" template that displays a grid of the "landing box" 
	content type for visitor navigation.
	
	This template uses the content-navbox-shadowed.php template to display
	landing boxes with colored shadows instead of colored text boxes.
	
	TEST FILE: Rather than duplicating nearly all of the code from 
	template-landing-page.php, it would be better to refactor and
			...
			<?php if( $boxes->have_posts() ) : ?>
				...
				<div class="entries">
					<?php while( $boxes->have_posts() ) : $boxes->the_post(); ?>
						<?php get_template_part('content', 'navbox-shadowed'); ?>
					<?php endwhile; ?>
				</div><!-- .entries -->
			<?php else : ?>
				<?php pinboard_404(); ?>
