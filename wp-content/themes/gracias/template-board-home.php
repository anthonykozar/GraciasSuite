<?php
/*  Template Name: Board Home

	Custom template that combines the content of a Page with a
	multi-column, archive-style display of all board members or just the
...
<?php get_header(); ?>
	...
	<div id="container">
		...
		<!-- Display board members per the assigned board(s) -->
		<section id="content" class="column onecol">
			<?php 
				...
				$boxes = new WP_Query(array('post_type' => 'grcs_board_member', 'tax_query' => $tax_parms, 'paged' => $paged, 'posts_per_page' => $posts_per_page));
			?>
			<?php if( $boxes->have_posts() ) : ?>
				<?php gracias_set_number_columns(3); ?>
				<div class="entries">
					<?php while( $boxes->have_posts() ) : $boxes->the_post(); ?>
						<?php get_template_part('content', 'grcs_board_member'); ?>
					<?php endwhile; ?>
				</div><!-- .entries -->
