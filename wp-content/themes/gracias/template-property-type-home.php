<?php
/*  Template Name: Property Type Home

	Custom template that combines the content of a Page with a
	multi-column, archive-style display of the property listings of the
...
<?php get_header(); ?>
	...
	<div id="container">
		...
		<!-- Display property listings per the assigned property type -->
		<section id="content" class="column onecol">
			<?php 
				...
				$prop_type = get_post_meta(get_the_ID(), 'wpcf-property-type', true);
				$tax_parms = array(
								array(
									'taxonomy' => 'grcs_property_type',
									'field'    => 'slug',
									'terms'    => $prop_type
								)
							);
				$posts_per_page = get_post_meta(get_the_ID(), 'wpcf-post-per-page', true);
				$boxes = new WP_Query(array('post_type' => 'grcs_proprty_listing', 'tax_query' => $tax_parms, 'paged' => $paged, 'posts_per_page' => $posts_per_page));
			?>
			<?php if( $boxes->have_posts() ) : ?>
				<?php gracias_set_number_columns(3); ?>
				<div class="entries">
					<?php while( $boxes->have_posts() ) : $boxes->the_post(); ?>
						<?php get_template_part('content', 'grcs_proprty_listing'); ?>
					<?php endwhile; ?>
				</div><!-- .entries -->
